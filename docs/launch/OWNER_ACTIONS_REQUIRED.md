...
## 3b. Run pending internal-CRM migrations 004-006 (safe, not urgent, run whenever convenient)
...
- **Status so far**: `packages/db/migrations/001-crm-foundation.sql`,
  ...
  (owner-confirmed during this session). Three more have been added
...
- **Action**: open each file in order (004, then 005, then 006) and run
  its full contents once in the SQL Editor. Each is additive-only (new
  tables, one additive column on `contacts`) and safe to run against the
  live production database - no existing table or row is altered. Each
...
- **Expected result**: `companies`, `notes`, and `tasks` tables exist
  ...
  once these are applied. The admin-console itself is not yet deployed
